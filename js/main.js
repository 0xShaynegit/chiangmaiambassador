...
function initNavigation() {
    ...
    dropdowns.forEach(dropdown => {
        ...
        // Mobile: group label click handler - show/hide links
        const menu = dropdown.querySelector('.nav-dropdown-menu')
        if (menu && window.innerWidth <= 640) {
            const groupLabels = menu.querySelectorAll('.nav-group-label')
            groupLabels.forEach(label => {
                label.style.cursor = 'pointer'
                const groupLinks = label.nextElementSibling
                if (groupLinks) {
                    groupLinks.style.display = 'none'
                }
                ...
                label.addEventListener('click', (e) => {
                    ...
                    // Close all other groups in this dropdown
                    menu.querySelectorAll('.nav-group-links').forEach(group => {
                        if (group !== links) {
