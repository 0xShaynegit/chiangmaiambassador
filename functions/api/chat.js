...
export async function onRequestPost(context) {
  ...
  if (contextChunks.length === 0) {
    return jsonResponse({
      answer:
        "I couldn't find anything about that on Chiang Mai Ambassador. Try rephrasing, or browse the site's guides directly.",
      sources: [],
    });
  }
  ...
  const systemPrompt = `You are the Chiang Mai Ambassador website assistant. Chiang Mai Ambassador is a Chiang Mai / Thailand living, food, visa, and lifestyle guide site. Answer the user's question using ONLY the context below, which is pulled from the site's own pages.
...
- Be proactive: after answering, suggest a relevant next step or related page the user probably wants next (e.g. a neighbourhood guide, a related food spot, a visa follow-up), if the context supports it.
- Use conversation history for follow-up questions ("how much does that cost?", "what about visas?").
